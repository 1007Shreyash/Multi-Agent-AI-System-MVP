...
try:
    ...
    # 3. Check for Groq Key (Switched from OpenAI)
    if "groq_api_key" not in st.secrets:
        st.error("🔥 CRITICAL STARTUP ERROR: Groq API Key not found.")
    ...
    if 'db' not in st.session_state:
        try:
            ...
            st.session_state.db = NotionManager(
                api_key=st.secrets["notion_api_key"],
                task_db_id=st.secrets["notion_task_db_id"],
                xp_db_id=st.secrets["notion_xp_db_id"]
            )
            print("Notion connected successfully.")
        except Exception as e:
            st.error(f"Failed to connect to Notion. Check secrets.toml. Error: {e}")
            st.stop()
    ...
    if 'calendar_manager' not in st.session_state:
        try:
            ...
            cal_creds = dict(st.secrets["google_calendar"])
            st.session_state.calendar_manager = CalendarManager(cal_creds)
        except Exception as e:
            st.warning(f"Calendar not connected: {e}")
    ...
    if 'email_manager' not in st.session_state:
        try:
            ...
            st.session_state.email_manager = EmailManager(
                email_creds["email_address"], 
                email_creds["app_password"]
            )
        except Exception as e:
            st.warning(f"Email not connected: {e}")
            ...
            if audio_bytes:
                ...
                try:
                    if 'whisper_agent' not in st.session_state:
                        ...
                        st.session_state.whisper_agent = WhisperAgent(api_key=st.secrets["groq_api_key"])
    ...
    with tab3:
        ...
        if task_history:
            ...
            # --- CHART LOGIC: Sort Oldest -> Newest ---
            # We create a specific list just for the chart, sorted by Task Number (1, 2, 3...)
            # This ensures the graph line goes UP from left to right.
            chart_data = sorted(task_history, key=lambda x: x['task_number'])
            ...
            # --- TABLE LOGIC: Keep Newest -> Oldest ---
            # We use the original 'task_history' list here because it comes 
            # from Notion sorted by Newest first.
            for i, task in enumerate(task_history[:10], 1):
                if task.get('created_at'):
                    # Check if it's a datetime object (Notion Manager v2) or string
                    if hasattr(task['created_at'], 'strftime'):
                        time_str = task['created_at'].strftime('%Y-%m-%d %H:%M:%S')
                    else:
                        time_str = str(task['created_at'])
                else:
                    time_str = "just now"
