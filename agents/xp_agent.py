...
class XPAgent:
    ...
    def add_xp(self, xp_earned, task_type):
        """
        Adds XP using the new Notion manager.
        """
        if not self.db:
            return self.get_stats()
            
        # 1. Map agent task types to PAEI personalities
        paei_map = {
            "research": "Producer",
            "report": "Producer",
            "email": "Administrator",
        ...
        personality = paei_map.get(task_type, "Integrator")

        # 2. Log to Notion
        self.db.log_xp(xp_earned, personality, f"Completed {task_type} task")
